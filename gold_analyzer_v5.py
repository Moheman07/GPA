...
def volume_profile(df: pd.DataFrame, bins: int = 20) -> Dict[str, Any]:
    try:
        ...
        ret = safe_pct_change(c)
        val_v = v_cap.reindex(ret.index)
...
def correlation_block(data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    try:
        ...
        if not dxy.empty:
            ...
            val = a.corr(b) if len(a) > 5 else None
            if pd.notna(val):
                out["USD"] = float(val)
        if not spy.empty:
...
def detect_divergences(df: pd.DataFrame, ind: Dict[str, pd.Series]) -> Dict[str, Any]:
    try:
        ...
        price = close.values
        pr_min, pr_max = np.nanmin(price), np.nanmax(price)
        pr_rng = pr_max - pr_min
        min_prom = max(0.002 * pr_rng, 0.5)
        if SCIPY_AVAILABLE:
            peaks, _ = find_peaks(price, prominence=min_prom)
            troughs, _ = find_peaks(-price, prominence=min_prom)
        else:
        ...
        for j in range(1, len(peaks)):
            ...
            if price[i1] > price[i0]:
                ...
                if pd.notna(r0) and pd.notna(r1) and r1 < r0:
                    add_div("bearish", "RSI", i1, price[i1], r0, r1)
                m0, m1 = macd.iat[i0], macd.iat[i1]
                if pd.notna(m0) and pd.notna(m1) and m1 < m0:
                    add_div("bearish", "MACD", i1, price[i1], m0, m1)
        for j in range(1, len(troughs)):
            i0, i1 = troughs[j - 1], troughs[j]
            if price[i1] < price[i0]:
                r0, r1 = rsi.iat[i0], rsi.iat[i1]
                if pd.notna(r0) and pd.notna(r1) and r1 > r0:
                    add_div("bullish", "RSI", i1, price[i1], r0, r1)
                m0, m1 = macd.iat[i0], macd.iat[i1]
                if pd.notna(m0) and pd.notna(m1) and m1 > m0:
                    add_div("bullish", "MACD", i1, price[i1], m0, m1)
...
def technical_sentiment(df: pd.DataFrame, ind: Dict[str, pd.Series]) -> Dict[str, Any]:
    ...
    r = float(rsi.iloc[-1]) if (not rsi.empty and pd.notna(rsi.iloc[-1])) else 50.0
...
def generate_signals(df: pd.DataFrame, ind: Dict[str, pd.Series]) -> Dict[str, Any]:
    ...
    if not s20.empty and not s50.empty and not s200.empty:
        if c > s20.iloc[-1] > s50.iloc[-1] > s200.iloc[-1]:
            out["trend"] = "صاعد قوي"
        elif c > s20.iloc[-1] > s50.iloc[-1]:
            out["trend"] = "صاعد"
        elif c < s20.iloc[-1] < s50.iloc[-1] < s200.iloc[-1]:
            out["trend"] = "هابط قوي"
        elif c < s20.iloc[-1] < s50.iloc[-1]:
            out["trend"] = "هابط"
        else:
    ...
    buy = sell = 0.0
    if out.get("rsi_signal") in ("شراء قوي", "شراء"):
        buy += 1.0 * (0.7 if weak else 1.0)
    if out.get("rsi_signal") in ("بيع قوي", "بيع"):
        sell += 1.0 * (0.7 if weak else 1.0)
    if out.get("macd_signal") == "شراء":
        buy += 1
    if out.get("macd_signal") == "بيع":
        sell += 1
    if out.get("bb_signal") == "شراء":
        buy += 0.8
    if out.get("bb_signal") == "بيع":
        sell += 0.8
    if out.get("trend") in ("صاعد", "صاعد قوي"):
        buy += 0.6
    if out.get("trend") in ("هابط", "هابط قوي"):
        sell += 0.6
    if buy > sell + 1:
    ...
    try:
        if not atr.empty and pd.notna(atr.iloc[-1]):
            ...
            out["stop_loss"] = float(c * 0.95)
            out["take_profit"] = float(c * 1.08)
...
def fundamentals_block() -> Dict[str, Any]:
    ...
    try:
        ...
        if fed.get("latest") and fed["latest"] > 3:
            bias -= 1
        if dxy.get("yoy_change") and dxy["yoy_change"] > 0.05:
            bias -= 1
        if cpi.get("yoy_change") and cpi["yoy_change"] > 0.03:
            bias += 1
    except Exception:
        pass
...
def backtest_macd_adx_bb_atr(df: pd.DataFrame, ind: Dict[str, pd.Series], params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    atr_sl = params.get("atr_mult_sl", 2.0)
    ...
    for i in range(2, len(c)):
        ...
        if cross_up and strong and not avoid_long:
            ...
            pos_side = "long"
            entry = price
            sl = price - atr_sl * atr.iat[i]
            tp = price + atr_tp * atr.iat[i]
    ...
    if len(trades) > 0:
        ...
        path = pd.Series(path)
        run_max = path.cummax()
        dd_series = (path - run_max) / run_max
