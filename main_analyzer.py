...
class ProfessionalGoldAnalyzer:
    ...
    def analyze_volume_profile(self, data):
        ...
        try:
            ...
            volume_analysis = {
                ...
                'volume_strength': 'قوي' if latest.get('Volume_Ratio', 1) > 1.5 
                                    else ('ضعيف' if latest.get('Volume_Ratio', 1) < 0.7 else 'طبيعي')
            }
    ...
    def generate_alerts(self, results):
        """توليد تنبيهات ذكية"""
        alerts = []
        gold_analysis = results.get('gold_analysis', {})
        current_price = gold_analysis.get('price_usd', 0)
        if current_price > 3450:
            alerts.append({
                'type': 'price_alert',
                'message': f'الذهب عند مستوى مرتفع: ${current_price}',
                'severity': 'medium'
            })
        volume = results.get('volume_analysis', {})
        if volume.get('volume_ratio', 0) > 3:
            alerts.append({
                'type': 'volume_alert',
                'message': f'حجم تداول عالي جداً: {volume.get("volume_ratio")}x',
                'severity': 'high'
            })
        if gold_analysis.get('signal') in ['Strong Buy', 'Strong Sell']:
            alerts.append({
                'type': 'signal_alert',
                'message': f'إشارة قوية: {gold_analysis.get("signal")}',
                'severity': 'high'
            })
        return alerts

    def track_trends(self, current_results, history_file='gold_history.json'):
        ...
        try:
            ...
            history.append({
                ...
                'price': current_results['gold_analysis']['price_usd'],
                'signal': current_results['gold_analysis']['signal'],
                'score': current_results['gold_analysis']['technical_score']
            })
            history = history[-100:]
            ...
            if len(history) >= 5:
                ...
                trend = 'صاعد' if recent_scores[-1] > recent_scores[0] else 'هابط'
                return {
                    'trend_direction': trend,
                    'score_change': recent_scores[-1] - recent_scores[0],
                    'consistency': len(set([h['signal'] for h in history[-3:]])) == 1
                }
            return {}
        except Exception as e:
            return {'error': str(e)}
    ...
    def get_economic_calendar(self):
        """جلب التقويم الاقتصادي (أمثلة توضيحية)"""
        important_events = [
            'FOMC Meeting',
            'CPI Release', 
            'NFP Report',
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # تتبع الترند وتحديث سجل النتائج
            results['trend_history'] = self.track_trends(results)
            # إضافة التقويم الاقتصادي
            results['economic_calendar'] = self.get_economic_calendar()
            # حفظ النتيجة في ملف واحد
...
def main():
    ...
    if results.get('status') != 'error' and 'gold_analysis' in results:
        ...
        if economic_calendar:
            print("\n📆 التقويم الاقتصادي:")
            upcoming = economic_calendar.get('upcoming_events', [])
            if upcoming:
                print("   • أحداث قادمة:")
                for event in upcoming:
                    print(f"       - {event}")
            print(f"   • التأثير المتوقع على الذهب: {economic_calendar.get('impact_on_gold', 'N/A')}")
    else:
